...
“灵动”是面向企业数据连接、智能分析与可信治理的产品品牌：

- **灵**：代表智能、敏锐与洞察，强调从企业数据和知识中快速发现价值。
- **动**：代表流动、连接与行动，强调能力协同、实时响应和结果落地。

灵动产品体系以 AI、数据资产、企业知识、智能分析和治理能力为基础，帮助企业连接可信能力、形成业务洞察并辅助决策。

### 灵动智策（LingDong Insight）

**产品定位：企业智能洞察与辅助决策平台。**

灵动智策面向企业经营分析、知识问答、智能研判和决策辅助场景，统一利用数据、知识与 AI 能力，为业务人员和管理者提供有证据、可追溯的洞察与分析结果。该产品名称替代原 **Apexsoft Insight**。

### 灵动智联（LingDong Nexus）

**产品定位：企业数据与智能能力连接平台。**

灵动智联负责统一连接和治理企业数据、知识、工具、应用与智能服务，为 AI Agent 提供安全、可信、可控、可观测的能力接入与协同基础。该产品名称替代原 **LiveMCP**，MCP 是其底层能力协议之一，不作为面向客户的主品牌名称。

两条产品线共同构成灵动体系：灵动智联提供企业智能能力的连接与治理底座，灵动智策基于这些可信能力完成企业洞察与辅助决策。
...
Agent Runtime / Agent OS 的 Kernel 第一职责就是可靠地把 Agent 程序跑起来，而不是代替 Agent 做业务判断，更不能因为模型一次判断不完整就停止整个程序。

合理的职责边界应该是：

- Agent 配置相当于“可执行程序”：工具、DAG、依赖、分支和失败策略。
- Java Runtime 相当于“内核”：计算 Ready Nodes、调度、状态持久化、租约、重试、超时、并发限制和故障恢复。
- LLM 相当于“用户态智能进程”：理解意图、审核候选、选择合法语义分支、补充参数和生成答案。
- MCP 工具相当于“驱动和系统调用”：提供资产、模板和执行结果。
- Reviewer 只是显式语义节点，不能升级为整个流程的隐式总开关。

因此 Kernel 应遵守一个核心原则：

> 只要节点满足配置依赖、权限、安全策略和输入合同，Runtime 就应继续执行；模型只能影响配置明确交给模型决策的部分。
...
“整体答案还不完整”“后续证据尚未获取”“模型认为还需要下一步”都不应该阻断当前已经合法完成的中间节点。
...
> 任何新增的模型调用，如果能够在配置未授权的情况下改变工具集合、依赖拓扑或终止固定工作流，就属于 Kernel 越权。
