...
- 模型可以总结证据，但不能替代证据本身。
- 工具结构化结果定义不可修改的事实边界；模型可以解释和归纳，但不能新增、改名、替换或否定工具事实。
- Runtime 必须在模型总结前注入权威事实块，并在总结后校验事实一致性。发现事实篡改时必须基于原始工具结果重写。
...
证据缺口首先触发补充检索、验证或计划改写，不直接触发拒答。只要任一 MCP 工具返回非空结果，最终答案必须分析该结果能够支持的部分，并将未覆盖维度列为限制；不得仅因结果不完整而写成“无法分析”或拒绝整个任务。只有完全没有可用事实、缺少高风险操作授权，或用户明确要求的精确结果无法取得时，才允许输出阻断说明。

## 回答证据状态表达

最终答案必须让用户第一眼看出回答性质：

- 有非空工具结构化结果、带来源文档证据、知识库证据或执行证据时，标记为 `有事实依据的分析`；证据不完整时输出阶段性分析并披露限制。
