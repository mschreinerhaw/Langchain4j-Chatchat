...
/** Typed Runtime OS protocol registry used as the composition boundary for replaceable ports. */
public final class RuntimeProtocolRegistry {
    ...
    public <P extends RuntimeProtocolPort> P require(Class<P> protocolType) {
        ...
        if (port == null) {
            throw new IllegalStateException("Runtime protocol is not registered: " + protocolType.getName());
        }
        return protocolType.cast(port);
    }

    public Map<Class<?>, RuntimeProtocolPort> ports() {
        return ports;
    }
    ...
    public static final class Builder {
        ...
        public <P extends RuntimeProtocolPort> Builder register(Class<P> protocolType, P implementation) {
            ...
            if (!protocolType.isInterface() || !protocolType.isInstance(implementation)) {
                throw new IllegalArgumentException("Implementation does not satisfy protocol "
                    + protocolType.getName());
            }
            if (ports.putIfAbsent(protocolType, implementation) != null) {
                throw new IllegalStateException("Runtime protocol already registered: "
                    + protocolType.getName());
            }
            return this;
        }

        public RuntimeProtocolRegistry build() {
            return new RuntimeProtocolRegistry(ports);
        }
    }
}
