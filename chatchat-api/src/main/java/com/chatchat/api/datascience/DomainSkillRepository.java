...
public interface DomainSkillRepository extends JpaRepository<DomainSkillEntity, String> {
    ...
    @Query("select distinct s.category from DomainSkillEntity s where s.tenantId = :tenantId order by s.category")
    ...
    List<DomainSkillEntity> findByTenantIdAndIdInAndStatus(String tenantId, List<String> ids, String status);

    List<DomainSkillEntity> findByTenantIdAndStatusOrderByPublishedAtDesc(String tenantId, String status);
}
