...
public class PythonCodeAssistantService {
    ...
    public AssistResponse assist(AssistRequest request) {
        ...
        String instruction = switch (action) {
            case "continue" -> "在保持已有实现和代码风格的前提下续写代码。只返回适合插入光标位置的新代码，不要重复上下文。";
            case "fix" -> "定位并修复选中代码或完整脚本中的问题。返回修复后的选中代码；没有选区时返回完整脚本。";
            case "optimize" -> "优化选中代码或完整脚本的可读性、健壮性和性能。返回优化后的选中代码；没有选区时返回完整脚本。";
    ...
    public record AssistRequest(String action, String prompt, String sourceCode, String selectedCode, String modelName) {}
