...
public class PythonDataFileEntity {
    ...
    @Column(name="file_type",length=24,nullable=false) private String fileType;
    @Column(name="file_size",nullable=false) private long fileSize;
    @Column(name="file_hash",length=64,nullable=false) private String fileHash;
    @JsonIgnore @Column(name="storage_path",length=1000) private String storagePath;
    @Column(name="python_path",length=1000,nullable=false) private String pythonPath;
    ...
    @PrePersist void create(){if(id==null)id=UUID.randomUUID().toString();Instant now=Instant.now();createdAt=now;updatedAt=now;if(status==null)status="TRANSFERRING";}
