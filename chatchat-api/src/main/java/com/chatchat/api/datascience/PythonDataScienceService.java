...
public class PythonDataScienceService {
    ...
    private String sha256(byte[] value){try{return HexFormat.of().formatHex(MessageDigest.getInstance("SHA-256").digest(value));}catch(Exception ex){throw new IllegalStateException(ex);}}
    private String safeDataFileName(String value){String name=value==null?"":value.trim();if(name.isBlank()||name.length()>255||name.contains("/")||name.contains("\\")||name.chars().anyMatch(Character::isISOControl))throw new IllegalArgumentException("数据文件名不合法");return name;}
    static String fileType(String name){int dot=name.lastIndexOf('.');String extension=dot<0?"":name.substring(dot+1).toLowerCase(Locale.ROOT);return switch(extension){case "csv"->"CSV";case "xls"->"XLS";case "xlsx"->"XLSX";case "json"->"JSON";case "txt"->"TXT";case "log"->"LOG";case "parquet"->"PARQUET";case "orc"->"ORC";case "zip"->"ZIP";default->throw new IllegalArgumentException("仅支持 CSV、XLS、XLSX、JSON、TXT、LOG、PARQUET、ORC、ZIP");};}
