...
public class PythonRuntimeService {
    ...
    public ProvisionResult provision(PythonAssetEntity asset) {
        try {
            ...
            if(inspect.exitCode()!=0){
                ...
                if(created.exitCode()!=0) return new ProvisionResult(false,container,workspace.toString(),created.stderr());
            }
        ...
        } catch(Exception ex){ return new ProvisionResult(false,containerName(asset.getId()),"",ex.getMessage()); }
    }
    ...
    public ExecResult execute(PythonAssetEntity asset,String fileName,String source,Map<String,Object> parameters) {
        ...
        try {
            ...
            if(!script.startsWith(workspace(asset).resolve("scripts"))) throw new IllegalArgumentException("非法脚本文件名");
        ...
        } catch(Exception ex){ return new ExecResult(-1,"",ex.getMessage(),0,false); }
    }
    ...
    private String containerName(String assetId){ return "chatchat-py-"+assetId.replaceAll("[^A-Za-z0-9_.-]","").toLowerCase(Locale.ROOT); }
    private String safe(String value){ return value==null?"default":value.replaceAll("[^A-Za-z0-9_.-]","_"); }
    private void requireSafeFileName(String name){ if(name==null||!name.matches("[A-Za-z0-9][A-Za-z0-9_.-]{0,170}\\.py")) throw new IllegalArgumentException("脚本文件名必须是安全的 .py 文件名"); }
    ...
    private ExecResult run(List<String> command,Duration timeout) throws IOException,InterruptedException {
        ...
        try {
            ...
            if(!finished){ process.destroyForcibly(); process.waitFor(5,TimeUnit.SECONDS); }
            return new ExecResult(finished?process.exitValue():124,get(out),get(err),elapsed(started),!finished);
        } finally { readers.shutdownNow(); }
    }
    private String readLimited(InputStream input) throws IOException { byte[] bytes=input.readNBytes(properties.getOutputLimitBytes()+1); String text=new String(bytes,0,Math.min(bytes.length,properties.getOutputLimitBytes()),StandardCharsets.UTF_8); return bytes.length>properties.getOutputLimitBytes()?text+"\n[输出已截断]":text; }
    private String get(Future<String> future){ try{return future.get(5,TimeUnit.SECONDS);}catch(Exception ex){return ex.getMessage();} }
