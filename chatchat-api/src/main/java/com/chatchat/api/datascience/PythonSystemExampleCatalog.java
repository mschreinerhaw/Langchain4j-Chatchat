...
public class PythonSystemExampleCatalog {
    ...
    public List<Example> list(){return examples;}
    public Example get(String id){return examples.stream().filter(e->e.id().equalsIgnoreCase(id)).findFirst().orElseThrow(()->new IllegalArgumentException("系统示例不存在"));}
    public byte[] data(String id){Example example=get(id);try{return new ClassPathResource("python-examples/data/"+example.dataFileName()).getInputStream().readAllBytes();}catch(IOException ex){throw new IllegalStateException("系统示例数据不可用："+example.dataFileName(),ex);}}

    private static Example table(String id,String format,String script,String data,String reader,List<String> requirements){return new Example(id,format,format+" 数据分析",script,data,"读取 "+format+" 数据，返回字段、行数、缺失值和数值汇总。",requirements,INPUT_SCHEMA,"""
        import json
        import os
        import pandas as pd
    ...
    private static Example text(String id,String format,String script,String data,boolean log){String analysis=log?"""
        import re
    ...
    private static Example zip(){return new Example("zip","ZIP","ZIP 压缩包检查","zip_analysis.py","sample_bundle.zip","安全检查 ZIP 内容并列出文件、压缩大小和未压缩大小。",List.of(),INPUT_SCHEMA,"""
        import json
        import os
        import zipfile
    ...
    public record Example(String id,String format,String name,String scriptFileName,String dataFileName,String description,List<String> requirements,String inputSchema,String sourceCode){}
}
