...
public class PythonTemplateIndexService {
    ...
    public IndexResult index(PythonTemplateEntity template){
        ...
        }catch(Exception ex){ log.warn("Python template indexing failed id={}: {}",template.getId(),ex.getMessage()); return new IndexResult(false,"FAILED",ex.getMessage()); }
    }

    public void remove(String templateId){
        if(!openSearchEnabled()) return;
        try(RestClient client=client()){ client.performRequest(new Request("DELETE","/"+properties.getIndexName()+"/_doc/"+templateId+"?refresh=true")); }
        catch(Exception ex){ log.warn("Python template index removal failed id={}: {}",templateId,ex.getMessage()); }
    }
    ...
    public List<SearchHit> search(String tenantId,String query,int limit){
        ...
        }catch(Exception ex){ log.warn("Python template hybrid search failed: {}",ex.getMessage()); return lexicalFallback(tenantId,query,limit); }
    }
    ...
    private List<JsonNode> hits(RestClient client,Map<String,Object> body)throws Exception{
        Request request=new Request("POST","/"+properties.getIndexName()+"/_search"); request.setJsonEntity(objectMapper.writeValueAsString(body));
        JsonNode root=objectMapper.readTree(client.performRequest(request).getEntity().getContent());
        List<JsonNode> hits=new ArrayList<>(); root.path("hits").path("hits").forEach(hits::add); return hits;
    }
    private void merge(Map<String,RankedHit> merged,List<JsonNode> hits,String channel){
        for(int rank=0;rank<hits.size();rank++){
            JsonNode hit=hits.get(rank),source=hit.path("_source"); String id=hit.path("_id").asText(); double rrf=1d/(60+rank+1);
    ...
    private List<SearchHit> lexicalFallback(String tenant,String query,int limit){
        ...
        return repository.findByTenantIdOrderByPublishedAtDesc(tenant).stream().filter(t->"PUBLISHED".equals(t.getStatus())).map(t->{
            String text=t.getSearchText().toLowerCase(Locale.ROOT); double score=Arrays.stream(terms).filter(text::contains).count();
    ...
    private Map<String,Object> document(PythonTemplateEntity t){ Map<String,Object> map=new LinkedHashMap<>(); map.put("templateId",t.getId()); map.put("tenantId",t.getTenantId()); map.put("ownerId",t.getOwnerId()); map.put("templateName",t.getTemplateName()); map.put("scenario",t.getScenario()); map.put("description",t.getDescription()); map.put("keywords",t.getKeywords()); map.put("domain",t.getDomain()); map.put("assetId",t.getAssetId()); map.put("scriptId",t.getScriptId()); map.put("version",t.getVersion()); map.put("status","PUBLISHED"); map.put("searchText",t.getSearchText()); map.put("publishedAt",t.getPublishedAt().toString()); return map; }
    private void ensureIndex(RestClient client)throws Exception{
        try{ client.performRequest(new Request("HEAD","/"+properties.getIndexName())); return; }catch(Exception ignored){}
        int dimension=searchProperties.getOpenSearch().getEmbedding().getDimension();
        Map<String,Object> fields=new LinkedHashMap<>(); for(String f:List.of("templateName","scenario","description","keywords","searchText")) fields.put(f,Map.of("type","text"));
        for(String f:List.of("templateId","tenantId","ownerId","assetId","scriptId","version","status","domain")) fields.put(f,Map.of("type","keyword"));
    ...
    private record RankedHit(String id,String name,String scenario,double score,LinkedHashSet<String> channels){ RankedHit add(double delta,String channel){ channels.add(channel); return new RankedHit(id,name,scenario,score+delta,channels); } }
}
