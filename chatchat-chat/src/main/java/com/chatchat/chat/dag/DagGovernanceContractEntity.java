...
@Immutable
@Table(
    name = "runtime_dag_governance_contract",
    indexes = @Index(
        name = "idx_dag_governance_contract_active",
        columnList = "contract_key, enabled, created_at"
    )
)
public class DagGovernanceContractEntity {
    ...
    @CollectionTable(
        name = "runtime_dag_governance_contract_rule",
        joinColumns = @JoinColumn(name = "contract_id", nullable = false),
        uniqueConstraints = @UniqueConstraint(
            name = "uk_dag_governance_contract_rule_path", columnNames = {"contract_id", "rule_path"}
        )
    )
