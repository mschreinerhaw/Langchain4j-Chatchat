...
public class McpPythonControlPlaneClient {
    ...
    public ExecutionResult executeTemplate(String id,Map<String,Object> parameters){return convert(post("/api/v1/python/templates/"+encode(id)+"/execute",parameters==null?Map.of():parameters),ExecutionResult.class);}
    ...
    private String encrypt(String source){String secret=credentials.resolvedSecret();if(secret.isBlank())throw new IllegalStateException("内部加密凭据未配置，禁止传输 Python 源码");return InternalSecretCipher.encrypt(source,secret);}
    ...
    public record EnvironmentView(String id,String name,String description,String dockerImage,String pythonVersion,String cpuLimit,String memoryLimit,int timeoutSeconds,boolean networkEnabled,int versionNumber,String status){}
    public record ProvisionResult(boolean ready,String containerName,String workspacePath,String message){}
    public record ExecutionResult(String id,String status,String stdout,String stderr,Integer exitCode,Long durationMs){}
    public record SyncResult(String id,String status,String toolName,String environmentId,String sourceHash){}
    public record TemplatePayload(String tenantId,String ownerId,String assetId,String environmentId,String templateName,String toolName,String version,String scenario,String description,String keywords,String domain,String inputSchemaJson,String outputSchemaJson,String source){}
}
