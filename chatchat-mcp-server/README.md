...
The MCP exposes one public search tool, `web_search`; the name describes its unified capability, not a preference for the public internet. Its primary sources are the local news index and locally collected governed financial data. MCP Server dynamically searches the `financial-data-asset` catalog and reads bounded rows from relevant datasets without relying on model-supplied `financial_data_required`, then searches the local news index. Financial retrieval executes first only so its evidence count can participate in the fallback decision; it does not outrank local news as a source class. A paid internet-search API is used only when the combined local evidence is insufficient (three evidence units by default, configurable with `chatchat.runtime.news.web-search.minimum-local-results`); `force-external` remains an explicit operational override. The Market module is packaged inside MCP Server and reuses MCP's DataSource, OpenSearch client and credentials. `financial_data_search` remains available for explicit dataset follow-up or expanded reads. `news_search` and `news_latest` remain internal runtime operations and are not published as MCP tools.
