...
### Financial query cache administration

The MCP admin **Cache Settings** page includes a dedicated `financial_data_search` tab. It lists active RocksDB and Redis entries with their dataset, normalized filters, date range, expiry, storage location, last-hit time and cumulative cache-hit query count. Operators can enable or disable this cache and change its storage, TTL, maximum entry size, Redis fallback and concurrent single-flight window. These settings are persisted in `mcp_financial_query_cache_config` and are enforced by the MCP financial query processing layer; Agent Runtime contains no financial cache policy.
...
The same observability is available for Capability Center database queries. Their existing per-template cache policy remains authoritative, while the database-query cache page now lists active entries with template, datasource, normalized parameters, tenant, storage, expiry, last-hit time and cumulative hit count. Hit statistics are persisted with each cache entry and returned in database-query tool metadata as `cacheHitCount` and `cacheLastHitAt`.
