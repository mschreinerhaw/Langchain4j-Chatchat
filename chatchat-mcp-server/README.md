...
The MCP exposes one public search tool, `web_search`; the name describes its unified capability, not a preference for the public internet. Its primary sources are the local news index and locally collected governed financial data. MCP Server passes the original `web_search` query into an internal financial-data stage, dynamically searches the `financial-data-asset` catalog, and reads bounded rows from relevant datasets before searching the local news index. `financial_data_required=true` forces this bridge stage but never creates a separate agent-planned financial tool call. Financial retrieval executes first only so its evidence count can participate in the fallback decision; it does not outrank local news as a source class. A paid internet-search API is used only when the combined local evidence is insufficient (three evidence units by default, configurable with `chatchat.runtime.news.web-search.minimum-local-results`); `force-external` remains an explicit operational override. Exact dataset follow-up is performed through `web_search(dataset=...)`. `financial_data_search`, `news_search`, and `news_latest` are internal operations and are not published as MCP tools.
...
The MCP admin **Cache Settings** page manages Capability Center database-query caches and automatic caches for built-in data sources in the same table. A source label distinguishes them; built-in financial data is one such row and has no separate business section, page or navigation tab. Expanding a row lists its active RocksDB and Redis entries with dataset or template, normalized filters, tenant, date range, expiry, storage location, last-hit time and cumulative cache-hit query count. Operators edit each source's enabled state, storage and TTL directly in that table; built-in-specific limits, Redis fallback and concurrent single-flight settings are contained in its expanded row. These settings are persisted in `mcp_financial_query_cache_config` and enforced by the MCP query processing layer; Agent Runtime contains no financial cache policy.

The module switch at the bottom of Cache Settings controls both Capability Center and built-in-source caching together and defaults to enabled for a new configuration. Existing persisted operator choices remain authoritative until changed and saved.

Changing the storage policy does not silently delete entries already held by the previous storage. The page provides explicit expired-entry cleanup and full eviction actions.

The same observability is available for Capability Center database queries. Their existing per-template cache policy remains authoritative. Configuration and maintenance are presented in one database-query cache table, with source labels and expandable runtime entries for normalized parameters, tenant, storage, expiry, last-hit time and cumulative hit count. Hit statistics are persisted with each cache entry and returned in database-query tool metadata as `cacheHitCount` and `cacheLastHitAt`.

Capability Center invocations pass the resolved MCP tenant and user scope explicitly into the query cache, including built-in financial query templates. If tenant context is unavailable, caching fails closed instead of using a shared fallback namespace; the tool result contains `cacheBypassReason=tenant_context_unavailable`, and the Cache Settings page reports the cumulative bypass count.
