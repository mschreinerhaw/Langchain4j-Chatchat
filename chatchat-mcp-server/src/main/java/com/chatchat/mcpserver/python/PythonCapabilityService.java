...
public class PythonCapabilityService {
    ...
    @Transactional public PythonExecution executeTemplate(String id,Map<String,Object> params){PythonTemplate t=templates.findById(id).orElseThrow(()->new IllegalArgumentException("Python 模板不存在"));if(!"PUBLISHED".equals(t.getStatus()))throw new IllegalArgumentException("Python 模板未发布");return execute(publishedEnvironment(t.getEnvironmentId()),t.getTenantId(),t.getOwnerId(),t.getAssetId(),t.getId(),"template_"+t.getId()+".py",decrypt(t.getSourceCiphertext()),params);}
