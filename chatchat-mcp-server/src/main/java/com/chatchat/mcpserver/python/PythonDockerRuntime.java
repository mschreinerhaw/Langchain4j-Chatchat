...
public class PythonDockerRuntime {
    ...
    public ProvisionResult provision(PythonEnvironment environment, String tenantId, String ownerId, String assetId) {
        try {
            ...
            if (inspect.exitCode() != 0)
                return new ProvisionResult(false, "", workspace.toString(), "MCP 节点不存在受控 Runtime 镜像：" + environment.getDockerImage());
            return new ProvisionResult(true, "", workspace.toString(), "Asset workspace 已就绪；执行时创建临时容器");
        } catch (Exception ex) {
            return new ProvisionResult(false, "", "", ex.getMessage());
        }
    }
    ...
    public ExecResult execute(PythonEnvironment env, String tenant, String owner, String asset, String fileName, String source, Map<String, Object> parameters) {
        if (fileName == null || !fileName.matches("[A-Za-z0-9][A-Za-z0-9_.-]{0,170}\\.py"))
            throw new IllegalArgumentException("非法 Python 文件名");
        ...
        try {
            ...
            Path assetWorkspace = workspace(tenant, owner, asset);
            Path scripts = assetWorkspace.resolve("scripts");
            Path inputRoot = dataFiles.uploads(tenant, owner);
            ...
            return new ExecResult(container, -1, "", ex.getMessage(), 0, false);
        } finally {
            try {
                run(List.of(properties.getDockerCommand(), "rm", "-f", container), Duration.ofSeconds(15));
    ...
    private void validateEnvironment(PythonEnvironment env) {
        ...
        if (!image.matches("[A-Za-z0-9][A-Za-z0-9._/:@-]{1,298}"))
            throw new IllegalArgumentException("Runtime 镜像名称不合法");
        if (image.endsWith(":latest") || (!image.contains(":") && !image.contains("@sha256:")))
            throw new IllegalArgumentException("Runtime 镜像必须固定版本，禁止 latest 或无标签镜像");
        if (!required(env.getRuntimeUser(), "运行用户").matches("[0-9]{1,10}(:[0-9]{1,10})?"))
            throw new IllegalArgumentException("运行用户必须是非 root UID[:GID]");
        if (env.getRuntimeUser().startsWith("0") && (env.getRuntimeUser().equals("0") || env.getRuntimeUser().startsWith("0:")))
            throw new IllegalArgumentException("禁止 root 用户运行 Python");
        if (!Set.of("NONE", "NAMED").contains(env.getNetworkPolicy()))
            throw new IllegalArgumentException("网络策略只能是 NONE 或 NAMED");
        if (env.getTimeoutSeconds() < 1 || env.getTimeoutSeconds() > 3600)
            throw new IllegalArgumentException("执行超时必须在 1 到 3600 秒之间");
        if ("NAMED".equals(env.getNetworkPolicy()) && (env.getNetworkName() == null || !env.getNetworkName().matches("[A-Za-z0-9][A-Za-z0-9_.-]{0,126}")))
            throw new IllegalArgumentException("NAMED 网络必须指定平台预建 Docker 网络");
        if (env.getCpuLimit() == null || !env.getCpuLimit().matches("[0-9]+(\\.[0-9]+)?"))
            throw new IllegalArgumentException("CPU 限制格式不合法");
        size(env.getMemoryLimit(), "内存");
        size(env.getDiskLimit(), "临时磁盘");
        size(env.getTmpfsLimit(), "tmpfs");
    }

    private void size(String value, String label) {
        if (value == null || !value.matches("[1-9][0-9]*(k|m|g)"))
            throw new IllegalArgumentException(label + "限制格式不合法");
    }
    ...
    private Path workspace(String tenant, String owner, String asset) throws IOException {
        Path root = Paths.get(properties.getWorkspaceRoot()).toAbsolutePath().normalize();
        Files.createDirectories(root);
        Path path = root.resolve(safe(tenant)).resolve(safe(owner)).resolve(safe(asset)).normalize();
        if (!path.startsWith(root)) throw new IllegalArgumentException("非法 workspace");
        return path;
    }
    ...
    private ExecResult run(List<String> command, Duration timeout) throws IOException, InterruptedException {
        ...
        try {
            ...
            return new ExecResult("", finished ? process.exitValue() : 124, get(stdout), get(stderr), TimeUnit.NANOSECONDS.toMillis(System.nanoTime() - started), !finished);
        } finally {
            readers.shutdownNow();
        }
    }
    ...
    private String read(InputStream stream) throws IOException {
        int limit = properties.getOutputLimitBytes();
        ByteArrayOutputStream kept = new ByteArrayOutputStream(Math.min(limit, 8192));
        byte[] buffer = new byte[8192];
        int total = 0, read;
        while ((read = stream.read(buffer)) >= 0) {
            if (total < limit) {
                int copy = Math.min(read, limit - total);
                kept.write(buffer, 0, copy);
            }
            total += read;
        }
    ...
    private String get(Future<String> f) {
        try {
    ...
    public record ExecResult(String containerId, int exitCode, String stdout, String stderr, long durationMs,
                             boolean timedOut) {
    }
}
