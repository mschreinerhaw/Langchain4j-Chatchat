...
```json
{
  ...
  "collectionDescription": "采集上市公司公告列表及公告详情正文，并解析 PDF 附件文本。",
...
- `sourceType` 只能是 `RSS`、`API`、`EXCHANGE_HOME`、`SZSE_HOME`、`HKEX_HOME`、`CSINDEX_HOME`、`CHINABOND_HOME`、`EXCHANGE_MARKET_DATA`、`EXCHANGE_DAILY_SNAPSHOT`、`SZSE_DISCLOSURE`、`STCN_DISCLOSURES`、`NEWS_HOME`、`CNINFO_HOME`、`CLS_TELEGRAPH`、`EASTMONEY_724`、`STRUCTURED_FLASH`、`CNINFO_ANNOUNCEMENTS`、`SSE_ANNOUNCEMENTS`、`WEB_LIST`、`WEB_SINGLE_PAGE`。

### 金融数据检索

- `web_search(query, num_results)`：同时检索新闻和金融数据业务目录，金融资产结果返回 `dataset`、业务描述、字段及受管存储位置。
- `web_search(dataset, filters, startDate, endDate, limit)`：只读查询目录注册数据集；`filters` 只能引用 Schema 注册字段，`limit` 最大为 200。
- 工具明确不接收原始 SQL，也不允许客户端指定任意数据库表。
- `entryUrl` 必须是合法 HTTP/HTTPS URL。
- `collectionDescription` 用于说明该 URL 实际采集的栏目、页数、日期范围以及正文/附件范围，最长 2000 字符；后台列表会直接展示该字段。
- `scheduleCron` 是六段 Spring Cron。
- 新建来源建议 `enabled=false`，验证规则后再启用。
- Agent 不得直接传递 WebMagic 线程数、抓取深度或任意底层执行参数。

`STRUCTURED_FLASH` 的最小模板配置示例：

```json
{
  "request": {
    "url": "https://api.example.com/flash",
    "signer": "NONE",
    "query": {"cursor": "${cursor}", "limit": "${pageSize}", "trace": "${timestamp}"}
  },
  "response": {
    "successPath": "code",
    "successValue": "1",
  ...
  "mapping": {
    ...
    "summary": "summary",
    "author": "author",
    "publishTime": "showTime",
    "publishTimeFormat": "yyyy-MM-dd HH:mm:ss",
    "sourceUrl": "https://example.com/news/${id}",
    "tagPaths": ["subjects.name"],
    "metadata": {"importance": "level"}
  },
  "compliance": {
    "categories": ["实时快讯"],
    "tags": ["示例来源"],
    "legalRisk": true,
    "legalDisclaimer": "内容仅用于内部资讯分析，不构成投资建议。"
  },
  "itemLimit": 50,
  "maxPagesPerRun": 200,
  "initialBackfillHours": 24,
  "initialCursor": "",
  "numericCursorBoundary": true,
  "sleepMillis": 300,
  "timeoutMillis": 20000,
  "zoneId": "Asia/Shanghai",
  "language": "zh-CN"
...
`request.signer` 只能引用服务端注册的白名单签名器；未知名称会在发送请求前失败。当前内置 `NONE`、`CLS_WEB`、`STCN_WEB`。时间格式支持 `EPOCH_SECONDS`、`EPOCH_MILLIS` 或 Java `DateTimeFormatter` 格式。

需要同时维护多个翻页参数时，可用复合分页状态：

```json
{
  "request": {
    "omitBlankQueryParameters": true,
    "query": {"page_time": "${state.page}", "last_time": "${state.cursor}"}
  },
  "response": {
    "itemsPath": "data",
    "nextState": {"page": "page_time", "cursor": "last_time"}
  },
  "initialState": {"page": "", "cursor": ""}
}
```

`mapping.tagPaths` 会递归展开任意层级的 JSON 数组，因此 `tags.name` 可同时提取主题标签和股票标签。复合分页只控制翻页；持久化断点仍使用资讯映射中的 `id:cursor`。

如果接口不在响应根节点提供下一页状态，可通过最后一条资讯生成翻页参数；同时可声明精确跳过条件：

```json
{
  "response": {
    "itemsPath": "data",
    "nextStateFromLastItem": {"cursor": "time"}
  },
  "mapping": {
    "skipWhen": {"data.lock": "true"}
  }
}
```

`skipWhen` 任一字段与配置值完全相等时跳过该条，不计为采集失败；它适合过滤广告或付费锁定占位项，不能用于绕过访问控制。

当上游一次返回完整列表且以编辑优先级等非时间字段排序时，设置 `"snapshotMode": true`。快照模式只请求一页、扫描全部条目、保留当前持久化游标，并依靠标准内容哈希去重；不能与需要翻页的接口混用。
...
面向 Agent 的统一检索能力。公开 MCP 只暴露该工具；`news_search`、`news_latest`、目录搜索和数据读取专用工具均不对外发布。

| 参数 | 类型 | 必填 | 默认/范围 |
| --- | --- | --- | --- |
| `query` | string | 条件必填 | 搜索时非空；读取 `dataset` 时可省略 |
| `num_results` | number | 否 | 默认 10，1～50 |
| `dataset` | string | 否 | 金融资产结果返回的数据集编码；传入后进入结构化数据读取模式 |
| `filters` | object | 否 | 仅允许 Schema 注册字段的精确过滤 |
| `startDate` / `endDate` | YYYY-MM-DD | 否 | 观测日期范围 |
| `limit` | number | 否 | 默认 50，1～200 |

搜索模式由 MCP Server 聚合，`data` 包含 `query`、`provider=chatchat-unified-search`、`mode=unified_news_and_financial_asset_search`、`count`、`newsCount`、`financialAssetCount`、`results`、`reference_urls`。读取模式由 MCP 进程内的 Market 能力返回目录信息、字段定义和有界观测行。
...
`web_search.results[]` 中的新闻结果使用统一证据结构：
