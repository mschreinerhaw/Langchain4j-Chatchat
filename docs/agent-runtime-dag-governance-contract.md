...
Runtime 不得根据具体表、字段、工具或业务关键词生成“符合/不符合”等业务结论，也不得用固定结论模板替换模型分析。最终答案修复采用 `model_analysis_repair_v1`：

1. 首次综合模型接收所有执行轮次的完整工具输出、权威语义投影、证据 ID、审核记录和未解决缺口，并生成候选分析。
2. Runtime 从同一执行快照构造二次分析上下文，不使用缩短后的 UI 摘要代替完整执行证据。
3. Reviewer 模型逐项核验候选答案中的实质性陈述；若发现无证据推断、从标识符推导语义或引入用户流程未声明的判断标准，必须基于同一执行证据生成完整修订答案。
4. `AnswerDecisionEngine` 只在 `modelEvidenceReviewRewriteAllowed=true` 且 Reviewer 返回 `REVISED` 时选择模型修订答案。Runtime 不参与业务结论生成。
5. 修订答案必须继续分析已返回的可用结果，不能退化为通用拒答；未解决内容由模型依据证据明确标记，不能由 Runtime 硬编码措辞。
