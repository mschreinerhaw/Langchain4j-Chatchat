...
| `parameter_readiness` | 参数能否由有效证据覆盖、由模板默认值满足，或作为无值的可选参数安全省略；仅“必填且无默认值”仍缺失时不就绪 |
...
3. `parameterSchema.required` 中的所有参数均已由可验证值或模板默认值满足；只有必填且无默认值的参数必须显式绑定。
...
任一门控失败时，当前模板不得调用执行工具并返回结构化错误；批量执行层不得因此终止其他满足门控的模板。无法映射、证据不足或校验无效的覆盖值不直接构成门控失败：有默认值时回退默认值，可选且无默认值时省略。
...
- 无法获得参数时应先使用模板默认值；只有必填且无默认值时，才要求用户补充或重选不需要该参数的模板。
...
| 缺少必填且无默认值的参数 | 仅阻断当前模板并返回结构化参数错误，或发布一次参数修复契约；同批其他模板继续执行 |
