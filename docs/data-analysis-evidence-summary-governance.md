...
7. 已发布 Agent 的名称、业务描述、业务场景和标签应编译为 `agent_role_analysis_context.v1`，固定到单次运行的独立分析管道，并贯穿 DAG 构建与重写、模板筛选与重选、证据投影、Worker、层级归并和最终综合。它只用于确定各阶段目标、相关性、分析重点和领域用语，不是返回数据、字段语义、计算授权或事实证据；工具载荷和普通请求属性不得覆盖该运行级上下文。
